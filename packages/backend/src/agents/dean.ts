...
async function llmVerdict(
  ...
  const prompt = [
    ...
    "",
    'IMPORTANT context — "Clanker", "Bankr" and "Virtuals" (Base) and',
    '"pump.fun" (Solana) are the names of trusted token launchpads. They are',
    'proper nouns, not English words — "Bankr" is NOT short for "bankrupt", and',
    '"Virtuals" is the Virtuals Protocol launchpad (a graduated AI-agent token),',
    'NOT the adjective "virtual". A token launched via any of them uses a',
    "standard, audited contract and LP setup, so contract-level honeypot or rug",
    "risk is already mitigated. Treat the launchpad identity as a positive",
