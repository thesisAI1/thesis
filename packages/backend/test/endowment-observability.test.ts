 ...
 *
 * Pins gaps G2, G3, G9, G13 from /tmp/logging-gaps.md.
 *
...
/** Chain that fails runLeg legs (buyback) but not sendEth for author.
...
test("G9: requestAuthorPayout tweet failure emits error ops event", async () => {
  ...
  // Use a normal chain so the buyback leg succeeds.
...
test("G13: payout-sent confirmation reply failure records a warn logEvent entry", async () => {
  ...
  // the non-silent path tries to post a "payout sent" tweet reply.
  ...
  try {
    ...
    // silentAuthorTweet: false → the payout-sent reply IS attempted.
    await runEndowment(
